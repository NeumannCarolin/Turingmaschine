...
if len(sys.argv) < 4: #ueberprueft auf BOM
    ...
    inp=inpu.split(',')
    #print(os.getcwd())
    filename=input("Gebe hier bitte den Filename der Turingmaschine ein "
                   "(z.B. algo4.py): ./filename :")
...
while my_file.is_file() == False and viewer != 'd' :
    viewer=input("Gebe hier einen existierenden Viewer mit Pfad an, "
                 "bei d wird der Defaultviewer benutzt : " )
    my_file = Path(str(viewer))
    print(my_file, my_file.is_file())
...
if turingmachine.num_tapes  == "2":
   ...
   for x in tm_commands:
    ...
    VTM=visual.VisualTM(inp, turingmachine.input_alphabet, turingmachine.gamma,
                        turingmachine.num_states, turingmachine.F, turingmachine.blank)
